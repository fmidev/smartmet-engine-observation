      ...
      if(!new_items.empty() && (block_full || last_item))
      {
        ...
        
        {
          ...
          for(std::size_t i = 0; i < insert_size; i++)
          {
            ...
            cmd.bind(":data_source", obs.data_source);
            cmd.execute();
            cmd.reset();
          }
                                           ...
                                           InsertStatus &insertStatus)
{
  try
  {
    ...
    for (std::size_t pos = 0; pos < flashCacheData.size(); ++pos)
    {
      // Abort if so requested
      if (itsShutdownRequested)
        return 0;

      const auto & item = flashCacheData[pos];
      const auto hash = item.hash_value();
      ...
      const bool block_full = (new_items.size() >= itsMaxInsertSize);
      const bool last_item = (pos == flashCacheData.size() - 1);
      ...
      if(!new_items.empty() && (block_full || last_item))
      {
        const auto insert_size = new_items.size();
        {
          ...
          for(std::size_t i = 0; i < insert_size; i++)
          {
            const auto & obs = flashCacheData[new_items[i]];

            std::string timestring = Fmi::to_iso_extended_string(obs.stroke_time);
            boost::replace_all(timestring, ",", ".");

            std::string created_timestring = Fmi::to_iso_extended_string(obs.created);
            boost::replace_all(created_timestring, ",", ".");

            std::string modified_last_timestring = Fmi::to_iso_extended_string(obs.modified_last);
            boost::replace_all(modified_last_timestring, ",", ".");

            std::string stroke_location = "GeomFromText('POINT(" +
                                          Fmi::to_string("%.10g", obs.longitude) + " " +
                                          Fmi::to_string("%.10g", obs.latitude) + ")', " + srid + ")";
            ...
            {
              cmd.bind(":timestring", timestring, sqlite3pp::nocopy);
              cmd.bind(":stroke_time_fraction", obs.stroke_time_fraction);
              cmd.bind(":flash_id", static_cast<int>(obs.flash_id));
              cmd.bind(":multiplicity", obs.multiplicity);
              cmd.bind(":peak_current", obs.peak_current);
              cmd.bind(":sensors", obs.sensors);
              cmd.bind(":freedom_degree", obs.freedom_degree);
              cmd.bind(":ellipse_angle", obs.ellipse_angle);
              cmd.bind(":ellipse_major", obs.ellipse_major);
              cmd.bind(":ellipse_minor", obs.ellipse_minor);
              cmd.bind(":chi_square", obs.chi_square);
              cmd.bind(":rise_time", obs.rise_time);
              cmd.bind(":ptz_time", obs.ptz_time);
              cmd.bind(":cloud_indicator", obs.cloud_indicator);
              cmd.bind(":angle_indicator", obs.angle_indicator);
              cmd.bind(":signal_indicator", obs.signal_indicator);
              cmd.bind(":timing_indicator", obs.timing_indicator);
              cmd.bind(":stroke_status", obs.stroke_status);
              cmd.bind(":data_source", obs.data_source);
...
ptime SpatiaLite::parseSqliteTime(const Fmi::DateTimeParser &parser, sqlite3pp::query::iterator &iter,
                                                     int column) const
{
  try
  {
