...
void MastQuery::setQueryParams(const MastQueryParams *qParams) {
  try {
    ...
    for (FieldMapType::const_iterator it = fv->begin(); it != fv->end(); ++it) {
      ...
      const std::shared_ptr<FieldAliasMapType> fam =
          qParams->getFieldAliasMap();
    ...
    for (OperationMapType::const_iterator omIt = om->begin(); omIt != om->end();
         ++omIt) {
    ...
    typedef MastQueryParams::JoinOnListTupleVectorType
    JoinOnListTupleVectorType;
    const std::shared_ptr<JoinOnListTupleVectorType> joinOnListTupleVector =
        qParams->getJoinOnListTupleVector();
    m_from.append(" ").append(qParams->getTableName()).append(" ").append(
        qParams->getTableName());

    if (joinOnListTupleVector->size() > 0) {
      JoinOnListTupleVectorType::const_iterator joinOnIt =
          joinOnListTupleVector->begin();
      for (; joinOnIt != joinOnListTupleVector->end(); ++joinOnIt) {
        ...
        for (std::list<MastQueryParams::NameType>::const_iterator joinField =
             ...
             joinField != std::get<2>(*joinOnIt).end(); joinField++) {
          m_from.append(joinField == std::get<2>(*joinOnIt).begin() ? " ON "
                                                                    : " AND ")
    ...
    const std::shared_ptr<OrderByVectorType> orderVector =
        qParams->getOrderByVector();
    for (OrderByVectorType::const_iterator it = orderVector->begin();
         it != orderVector->end(); ++it) {
