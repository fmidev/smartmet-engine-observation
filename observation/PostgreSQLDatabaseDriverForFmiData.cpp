...
TS::TimeSeriesVectorPtr PostgreSQLDatabaseDriverForFmiData::values(Settings &settings)
{
  ...
  try
  {
    ...
    if (tablename == WEATHER_DATA_QC_TABLE)
      return db->getLocationDataItems(stations, settings, *info, timeSeriesOptions, itsTimeZones);
    if (tablename == FLASH_DATA_TABLE)
    ...
    const std::string &producer, const Fmi::DateTime &from, const MeasurandInfo &minfo) const
{
  try
  {
    ...
    if (cache)
    {
      auto cache_table = DatabaseDriverBase::resolveCacheTableName(
          producer, itsParameters.params->stationtypeConfig);
      ret = cache->getLatestDataUpdateTime(cache_table, from, producer_ids, measurand_ids);
    }

    if (ret.is_not_a_date_time())
    {
      auto database_table = DatabaseDriverBase::resolveDatabaseTableName(
          producer, itsParameters.params->stationtypeConfig);
