...
ObservationCacheAdminPostgreSQL::ObservationCacheAdminPostgreSQL(
    ...
    : ObservationCacheAdminBase(p, geonames, conn_ok, timer),
      itsPostgreSQLConnectionPool(pcp)
{
  // Locations area loaded in the same driver
  if (itsParameters.loadStations)
  {
    logMessage(
        "[PostgeSQLDatabaseDriver] Loading locations from PostgreSQL database...", itsParameters.quiet);
    ...
    logMessage("PostgreSQLDatabaseDriver] Locations read from PostgreSQL database.", itsParameters.quiet);
  }
}
  
void ObservationCacheAdminPostgreSQL::readObservationCacheData(std::vector<DataItem>& cacheData,
...
void ObservationCacheAdminPostgreSQL::readFlashCacheData(std::vector<FlashDataItem>& cacheData,
														 const boost::posix_time::time_period& dataPeriod,
														 const Fmi::TimeZones& timezones) const
{
  boost::shared_ptr<PostgreSQLObsDB> db = itsPostgreSQLConnectionPool->getConnection();
...
void ObservationCacheAdminPostgreSQL::readWeatherDataQCCacheData(std::vector<WeatherDataQCItem>& cacheData,
																 const boost::posix_time::time_period& dataPeriod,
																 const std::string &fmisid,
																 const std::string &measurandId,
																 const Fmi::TimeZones& timezones) const
{
  boost::shared_ptr<PostgreSQLObsDB> db = itsPostgreSQLConnectionPool->getConnection();
  db->readWeatherDataQCCacheDataFromPostgreSQL(cacheData, dataPeriod, fmisid, measurandId, itsTimeZones);
}
...
{
  boost::shared_ptr<PostgreSQLObsDB> db = itsPostgreSQLConnectionPool->getConnection();
  db->readFlashCacheDataFromPostgreSQL(cacheData, startTime, lastStrokeTime, lastModifiedTime, itsTimeZones);
}
    ...
    const Fmi::TimeZones& timeZones) const
{
  ...
  if(producer == FMI_IOT_PRODUCER)
	{
	  const PostgreSQLDriverParameters& p = static_cast<const PostgreSQLDriverParameters&>(itsParameters);
		  ...
		  if (p.fmiIoTStations->isActive(*item.station_code, item.data_time))
			{
			  ...
			  if (s.elevation >= 0.0) item.altitude = s.elevation;
			}
		}
	}
}
    ...
    const boost::shared_ptr<ObservationCache>& cache) const
{
  ...
  if (last_modified_time.is_not_a_date_time()) last_modified_time = last_time;
...
std::map<std::string, boost::posix_time::ptime>
ObservationCacheAdminPostgreSQL::getLatestFlashTime(
    const boost::shared_ptr<ObservationCache>& cache) const
{
  ...
  if (last_modified_time.is_not_a_date_time()) last_modified_time = start_time;
  if (last_stroke_time.is_not_a_date_time()) last_stroke_time = start_time;  
