...
TS::TimeSeriesVectorPtr CommonPostgreSQLFunctions::getMagnetometerData(const Settings &settings,
																		 const Fmi::TimeZones &timezones)
{
																	   ...
																	   const Fmi::TimeZones &timezones)
{
  ...
  std::string sqlStmt = "SELECT station_id, magnetometer, level, EXTRACT(EPOCH FROM date_trunc('seconds', data_time)) AS obstime, "
	"x as magneto_x, y as magneto_y, z as magneto_z, t as magneto_t, f as magneto_f, data_quality ";
  if(starttime == endtime)
	sqlStmt += ("from magnetometer_data where data_time = '" + starttime + "'");
  else
	sqlStmt += ("from magnetometer_data where (data_time >= '" + starttime + "' AND data_time <= '" + endtime + "')");
