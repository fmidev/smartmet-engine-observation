...
{
  if(itsIntegerToString.find(int_value) == itsIntegerToString.end())
	return missing_string;

  return itsIntegerToString.at(int_value);
}

int RoadAndForeignIds::stringToInteger(const std::string& string_value) const
{
  if(itsStringToInteger.find(string_value) == itsStringToInteger.end())
	return missing_integer;

  return itsStringToInteger.at(string_value);
}
